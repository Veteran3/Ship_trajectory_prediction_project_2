...
class StructureAwareEmbedding(nn.Module):
    """
    V3.9.0 结构感知嵌入层
    核心作用：在源头防止特征污染。
    - 动态特征 (0~6): 显式提取物理微分特征 (Vel, Acc)。
    - 静态特征 (7~16): 仅做线性映射，绝不进行微分操作。
    """
    def __init__(self, num_dynamic, num_static, d_model, dropout=0.1):
        super().__init__()
        
        # 1. 维度分配策略
        # 建议: 75% 的维度给动态物理特征，25% 给静态上下文
        self.d_dyn = int(d_model * 0.75) 
        self.d_stat = d_model - self.d_dyn
        
        print(f"Embedding Split: Dynamic={self.d_dyn} dims, Static={self.d_stat} dims")
        
        # -----------------------------------------------------------
        # Stream A: 动态流 (Dynamic Stream) - 物理感知
        # -----------------------------------------------------------
        self.dyn_input_proj = nn.Linear(num_dynamic, self.d_dyn)
        
        # 物理算子分支 (仅对动态特征操作)
        # 1. 位置 (Identity)
        self.branch_pos = nn.Conv1d(self.d_dyn, self.d_dyn, kernel_size=3, padding='same', groups=self.d_dyn)
        # 2. 速度 (Velocity)
        self.branch_vel = nn.Conv1d(self.d_dyn, self.d_dyn, kernel_size=3, padding='same', groups=self.d_dyn, bias=False)
        # 3. 加速度 (Acceleration)
        self.branch_acc = nn.Conv1d(self.d_dyn, self.d_dyn, kernel_size=3, padding='same', groups=self.d_dyn, bias=False)
        
        # 初始化物理权重
        self._init_physics_weights()
        
        # 动态特征融合: [Pos, Vel, Acc] -> [Dynamic_Embedding]
        self.dyn_fusion = nn.Linear(3 * self.d_dyn, self.d_dyn)

        # -----------------------------------------------------------
        # Stream B: 静态流 (Static Stream) - 语义保留
        # -----------------------------------------------------------
        # 静态特征不需要卷积，不需要微分，直接映射
        self.stat_input_proj = nn.Linear(num_static, self.d_stat)
    ...
    def forward(self, x):
        """
        Input x: [B, N, T, Total_Feats] (例如 17维)
        """
        B, N, T, F = x.shape
        
        # 1. 显式切分特征
        # 假设前 7 维是动态 (Lat, Lon, SOG, COG, Heading, L, W)
        # 后面 10 维是静态 (Lane_OneHot, Dir)
        x_dyn_raw = x[..., :self.dyn_input_proj.in_features] 
        x_stat_raw = x[..., self.dyn_input_proj.in_features:] 
        
        # --- 处理动态流 ---
        # [B, N, T, D_dyn]
        h_dyn = self.dyn_input_proj(x_dyn_raw)
        
        # Conv1d 需要 [Batch, Channel, Time] -> [B*N, D_dyn, T]
        h_dyn_in = h_dyn.reshape(B*N, T, -1).transpose(1, 2)
        
        # 物理提取 (这里只会有真正的物理量参与计算)
        feat_pos = self.branch_pos(h_dyn_in).transpose(1, 2)
        feat_vel = self.branch_vel(h_dyn_in).transpose(1, 2)
        feat_acc = self.branch_acc(h_dyn_in).transpose(1, 2)
        
        # 融合动态特征
        cat_dyn = torch.cat([feat_pos, feat_vel, feat_acc], dim=-1)
        emb_dyn = self.dyn_fusion(cat_dyn) # [B*N, T, D_dyn]
        
        # --- 处理静态流 ---
        # [B, N, T, D_stat]
        h_stat = self.stat_input_proj(x_stat_raw)
        emb_stat = h_stat.reshape(B*N, T, -1) # [B*N, T, D_stat]
        
        # --- 拼接隔离流 ---
        # 此时: 前 75% 的通道是纯粹的物理信息，后 25% 是纯粹的静态信息
        emb_total = torch.cat([emb_dyn, emb_stat], dim=-1) # [B*N, T, D_model]
        
        # 还原维度
        emb_total = emb_total.view(B, N, T, -1)
        
        return self.dropout(self.norm(emb_total))
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.num_dynamic = 7   # 0-6: 经纬度、速度、航向等
        self.num_static = 10   # 7-16: 航道ID、方向等
        # -- 嵌入层 --
        # =========================================================================
        # 修改：输入端物理隔离，隔离动态和静态特征，防止特征污染
        # =========================================================================

        self.src_input_proj = StructureAwareEmbedding(
